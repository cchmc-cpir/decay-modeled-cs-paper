...
Author: 
    Xavier Hoyos - franciscoxavier.hoyoscordon@cchmc.org

Adapted from:
    Joseph Plummer - joseph.plummer@cchmc.org 
    'gasex_recon_3d' repository
...
# Folder
folder = "/mnt/c/Users/hoyeh3/Desktop/CCHMC-Gas-Exchange-Processing-Package-master/Test20191125 IRC186H-289"
...
# subset_type = "quarter"  # Divide the FID into quarters
edge_1 = 25
edge_2 = 50
...
data_file = loadmat(os.path.join(folder, "GasExchangeWorkspace.mat"))['GasKSpace']
coords_file = loadmat(os.path.join(folder,"GasExchangeWorkspace.mat"))['XeTraj']

# Load data (channels, projections, samples)
data = np.expand_dims(data_file.T,axis=0)
...
if subset_type == "every other n":
    subset = int(100 / subsample_pc_gas)
    data_subset = data[:, steady_state::subset, :]
    coords_subset = coords_resize[steady_state::subset, ...]

if subset_type == "quarter":
    # Steady state
    edge = int(N_proj * edge_1 / 100)
    subset = int(N_proj * edge_2 / 100)
    data_subset = data[:, edge:subset, :]
    coords_subset = coords_resize[edge:subset, ...]
...
# Visual of projections used (only for plot)
# visual_projections = np.linspace(edge,subset,data_subset.shape[1])

# Visualize
plt.figure()
plt.plot(abs(data[0, :, 0]), color='r')
# plt.plot(visual_projections,abs(data_subset[0, :, 0]), color='g')
plt.ylabel('$k_0$ intensity')
plt.title("magnetization decay, percentage: %i" %subsample_pc_gas)
...
data_file = loadmat(os.path.join(folder, "GasExchangeWorkspace.mat"))['DissolvedKSpace']
coords_file = loadmat(os.path.join(folder,"GasExchangeWorkspace.mat"))['XeTraj']
...
ax2.axis('off')
ax3.axis('off')
ax1.set_title('CG gas_x')
ax2.set_title('log(CG gas_x)')
ax3.set_title('NUFFT vs. CG')
...
ax3.axis('off')
ax1.set_title('CG gas_x')
ax2.set_title('SNR CG gas_x')
ax3.set_title('angle CG gas_x')
...
ax1.axis('off')
ax1.set_title('NUFFT (dissolved/gas exchange)')
...
# fig.colorbar(img_plot1)
ax1.axis('off')
ax1.set_title('CG (dissolved/gas exchange)')
...
if use_dcf:
    fig.suptitle('max_iter=%i, \u03C1=%.1e, $p_{tol}$=%.1e, $A^TA$=%i, \n \u03BB$_1$=%.1e, \u03BB$_2$=%.1e, \u03BB$_3$=%.1e, \zeta=%.1f, \n \u03BB$_4$=%.1e, $\eta$=%.1e' % (
...
# sub_folder = os.path.join(folder, r'XH_CS_recon')
# 
# try:
#     os.mkdir(sub_folder)
#     print("Folder %s created!" % sub_folder)
# except FileExistsError:
#     print("Folder %s already exists" % sub_folder)
# 
# Save gas exchange
# Gridded recon
# ni_img = nib.Nifti1Image(np.rot90(np.rollaxis(abs(img_nufft_gas),axis=2),k=3), affine=np.eye(4))
# ni_img = nib.Nifti1Image(np.flip(np.rot90(np.rollaxis(abs(img_nufft_gas),axis=2),k=3),axis=2), affine=np.eye(4))
# nib.save(ni_img, os.path.join(sub_folder, 'img_nufft_gas.nii.gz'))

# CS with CG
# ni_img = nib.Nifti1Image(abs(img_cg_gas), affine=np.eye(4))
# nib.save(ni_img, os.path.join(sub_folder, 'img_cg_gas.nii.gz'))

# # CS with ADMM
# ni_img = nib.Nifti1Image(abs(img_cs_cg_gas), affine=np.eye(4))
# nib.save(ni_img, os.path.join(sub_folder, 'img_cs_cg_gas.nii.gz'))

# # Save dissolved phase
# # Gridded recon
# ni_img = nib.Nifti1Image(abs(img_nufft_dissolved), affine=np.eye(4))
# nib.save(ni_img, os.path.join(sub_folder, 'img_nufft_dissolved.nii.gz'))

# # CS with CG
# ni_img = nib.Nifti1Image(abs(img_cg_dissolved), affine=np.eye(4))
# nib.save(ni_img, os.path.join(sub_folder, 'img_cg_dissolved.nii.gz'))

# # CS with ADMM
# ni_img = nib.Nifti1Image(abs(img_cs_cg_dissolved), affine=np.eye(4))
# nib.save(ni_img, os.path.join(sub_folder, 'img_cs_cg_dissolved.nii.gz'))
